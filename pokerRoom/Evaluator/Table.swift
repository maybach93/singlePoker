...
public struct Table {
    ...
    public func bestHandWithCards(cards: [Card]) -> Hand? {
        guard let c = try? combineCards(cards: self.cards + cards) else { return nil }
        
        let orderedHands = c.sorted { bestHand(hand1: Hand(cards: $0), Hand(cards: $1)) == Hand(cards: $0) }
...
func combineCards(cards: [Card]) throws -> [[Card]] {
    let combinations = try combine(n: cards.count, 5)
